...
function setRechargeRebate1($tid, $money,$reward,$bz='')
    ...
    if($bz=='个人充值奖励'){
        $bz=$bz;
        $bz_zh_hk = 'Tiền thưởng cá nhân';
        $bz_en_us = 'Individual recharge rewards';
    }else if($bz=='团队奖励'){
        $bz=$bz;
        $bz_zh_hk = 'Giải thưởng nhóm';
        $bz_en_us = 'Team rewards';
    }else{
        $bz="下级".$bz."会员返佣";
        $bz_zh_hk = 'Cấp thấp'.$bz."Thành viên trở lại của tôi";
        $bz_en_us = "Subordinate ".$bz." member rebates";
    }
        $rebate = round($reward * $money / 100, 2);
        if (0 < $rebate) {
            $LcTips173 = Db::name('LcTips')->where(['id' => '173']);
