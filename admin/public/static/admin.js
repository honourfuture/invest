...
$(function () {
            ...
            this.load(url, data, 'GET', function (res, index) {
                ...
                index = layer.open({
                    type: 1, btn: false, area: "800px", content: res, title: title || '', success: function (dom, index) {
                        ...
                        $(dom).find('.layui-googleauth-btn').off('click').on('click', function () {
                          if(!$(dom).find('input[name="googleauth"]').is(":checked") ) {
                             var codeTips = layer.open({
                                type: 1, 
                                btn: false, 
                                area: "600px", 
                                content: '<style></style><form class="layui-form" action=""><div id="virtual-tMzWJ"> <div class="bt-form pd25"><div class="mb15"><i class="layui-layer-ico layui-layer-ico3 layer-info-ico"></i><h3 class="layer-info-title">风险操作！此功能不懂请勿开启！</h3></div><ul class="help-info-text c7 explainDescribeList pd15"><li style="color: red;">必须要用到且了解此功能才决定自己是否要开启!</li><li style="color: red;">如果无法验证，数据库删除验证码, 取消动态口令认证</li><li>开启服务后，请立即绑定，以免出现面板不能访问。</li><li>请先下载宝塔APP或(谷歌认证器)，并完成安装和初始化。</li><li><span>基于google Authenticator 开发,如遇到问题请 <a class="btlink undefined" href="https://www.bt.cn/bbs/forum.php?mod=viewthread&amp;tid=105839" target="_blank">查看详情</a></span></li></ul><div class="download_Qcode"><div class="item_down"><div class="qcode_title">Android/IOS应用 扫码下载</div><div class="qcode_conter"><img id="qr-google-code" src="" style="width: 180px;height: 180px;"></div></div></div><div class="mt10 plr15 agreementBox" id="checkAuthenticator"> <input type="checkbox" lay-skin="primary" name="agreement_more" title="我已安装APP和了解详情,并愿意承担风险！"></div></div></div> <div class="layui-form-item text-center"><button class="layui-btn" type="button"  data-code-save>开启</button><button class="layui-btn layui-btn-danger" type="button" data-confirm="确定要取消操作吗？" data-code-close>取消</button></div><input type="hidden" name="secret" value="0"> </form>', 
                                title: '设置动态口令认证',
                                success: function (dom, index) {
                                    var  loadX =  layer.msg('加载中', { icon: 16 , shade: 0.01 }); 
                                    $.ajax({ 
                                        data: {t: Date.parse(new Date())}, type: 'post', url:'/admin/user/qrcode', 
                                        error: function (XMLHttpRequest) {
                                           $.msg.tips('E' + XMLHttpRequest.status + ' - 服务器繁忙，请稍候再试！');
                                        }, success: function (ret) {
                                            if(ret.code == 1 ) {
                                                $(dom).find('#qr-google-code').attr('src', ret.info.image);
                                                $(dom).find('input[name="secret"]').val(ret.info.secret);
                                            } else {
                                                $.msg.tips('服务器繁忙，请稍候再试！');
                                            }
                                        }, complete: function () {
                                           layer.close(loadX);
                                        }
                                    ...
                                    $(dom).find('[data-code-save]').off('click').on('click', function () {
                                        if( $(dom).find('input[name="agreement_more"]').is(":checked") == false){
                                            layer.tips('请阅读并同意承担风险', $(dom).find('#checkAuthenticator'), {tips: 1});
                                            return false;
                                        }
                                        var  bcloadX = layer.msg('提交中', { icon: 16 , shade: 0.01 }); 
                                        $.ajax({ 
                                            data: {t: Date.parse(new Date()), secret:$(dom).find('input[name="secret"]').val()}, type: 'post', url:'/admin/user/opensecret', 
                                            error: function (XMLHttpRequest) {
                                               $.msg.tips('E' + XMLHttpRequest.status + ' - 服务器繁忙，请稍候再试！');
                                            }, success: function (ret) {
                                                if(ret.code == 1 ) {
                                                    layer.close(bcloadX);
                                                    layer.close(codeTips);
                                                    $(zIndexDom).find('input[name="googleauth"]').prop("checked", true);
                                                    window.form.render();
                                                    layer.msg('谷歌认证开启成功。');
                                                } else if(ret.code == 0 ) {
                                                    layer.close(bcloadX);
                                    ...
                                    $(dom).find('[data-code-close]').off('click').on('click', function () {
                                        layer.close(codeTips);
                                    });
                                    window.form.render();
                           ...
                           $.msg.confirm('您已开启谷歌认证，确定要关闭认证吗?', function (_index) {
                                layer.close(_index);
                                
                                 var  xbcloadX = layer.msg('提交中', { icon: 16 , shade: 0.01 }); 
                                        $.ajax({ 
                                            data: {t: Date.parse(new Date())}, type: 'post', url:'/admin/user/closesecret', 
                                            error: function (XMLHttpRequest) {
                                               $.msg.tips('E' + XMLHttpRequest.status + ' - 服务器繁忙，请稍候再试！');
                                            }, success: function (ret) {
                                                if(ret.code == 1 ) {
                                                    layer.close(xbcloadX);
                                                    //$(zIndexDom).find('.layui-googleauth-btn').text("动态口令配置");
                                                    $(zIndexDom).find('input[name="googleauth"]').prop("checked", false);
                                                    window.form.render();
                                                    layer.msg('谷歌认证关闭成功。');
                                                }  else {
                                                    layer.close(xbcloadX);
    ...
    $.fn.uploadOneVideo = function () {
        var name = $(this).attr('name') || 'image', type = $(this).data('type') || 'mp4,jpg,gif';
        var $tpl = $('<a data-file="btn" class="uploadimage">上传视频</a>').attr('data-field', name).attr('data-type', type);
