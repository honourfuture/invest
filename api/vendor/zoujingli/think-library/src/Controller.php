...
abstract class Controller extends \stdClass
{
    ...
    protected function checkToken(){
        // echo $this->request->param('token');exit;
        if(!$this->request->param('token'))
        {
            $tips = Db::name('LcTips')->where(['id' => '98']);
            $returnData = array(
                "zh_cn" =>$tips->value('zh_cn'),
                "zh_hk" =>$tips->value('zh_hk'),
                "en_us" =>$tips->value('en_us'),
                "th_th" =>$tips->value('th_th'),
                "vi_vn" =>$tips->value('vi_vn'),
                "ja_jp" =>$tips->value('ja_jp'),
                "ko_kr" =>$tips->value('ko_kr'),
                "ms_my" =>$tips->value('ms_my'),
            );
            $this->error($returnData,'',403);
        }
        try{
            $result = JWT::decode($this->request->param('token'), '2nUS[-TH^mL{dW3N>ZAfaJ:z&4l+jsXoCy@h0PI~', array('HS256'));//进行解密算法
            $this->userInfo = (array)$result->data;
            $user = Db::name('lc_user')->find($this->userInfo['id']);
            
            if (!$user['clock']) {
                $this->error($user['clock_msg']);
            }
            if ($user['token'] != $this->request->param('token')) {
                $tips = Db::name('LcTips')->where(['id' => '98']);
                $returnData = array(
                    "zh_cn" =>$tips->value('zh_cn'),
                    "zh_hk" =>$tips->value('zh_hk'),
            ...
            $data = ['logintime' => time(),'id'=>$this->userInfo['id']];
            Db::name('LcUser')->update($data);
        }catch(\Exception $exception){
            $tips = Db::name('LcTips')->where(['id' => '99']);
            $returnData = array(
                "zh_cn" =>$tips->value('zh_cn'),
                "zh_hk" =>$tips->value('zh_hk'),
                "en_us" =>$tips->value('en_us'),
    ...
    public function getToken($user){
        $token = array(
            ...
            'exp' => time() + 3600*168,           //过期时间
            'data' => array(
                'id'    =>  $user['id'],
                'phone'  =>  $user['phone'],
            )
        );
        $jwt = JWT::encode($token, '2nUS[-TH^mL{dW3N>ZAfaJ:z&4l+jsXoCy@h0PI~');
        return $jwt;
    }
    ...
    private function decryptToken($jwtToken){
        try{
            $result = JWT::decode($jwtToken, '2nUS[-TH^mL{dW3N>ZAfaJ:z&4l+jsXoCy@h0PI~', array('HS256'));//进行解密算法
    ...
    protected function _formBank($dbQuery, $template = '', $field = '', $where = [], $data = [])
    {
        return FormHelper::instance()->initBank($dbQuery, $template, $field, $where, $data);
    }


    protected function _formUser($dbQuery, $template = '', $field = '', $where = [], $data = [])
    {
        return FormHelper::instance()->initUser($dbQuery, $template, $field, $where, $data);
    }
